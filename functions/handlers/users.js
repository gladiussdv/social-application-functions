...
exports.signup = (req, res) => {
  ...
  db.doc(`/users/${newUser.handle}`).get().then(doc => {
    ...
    const userCredentials = {
      ...
      imageUrl: `https://firebasestorage.googleapis.com/v0/b/${config.storageBucket}/o/${profileimg}?alt=media`,
      userId
    }
...
exports.login = (req, res) => {
  ...
  firebase.auth().signInWithEmailAndPassword(user.email, user.password).then(data => {
    ...
    // auth/user-not-user
    console.error(err)
    return res.status(403).json({ general: 'Wrong credetials, please try again' })
  })
}
